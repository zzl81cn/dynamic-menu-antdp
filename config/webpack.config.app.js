...
function initWebpackSetting(webpackConfig) {
  ...
  webpackConfig.output.publicPath =
    ...
    webpackConfig.module.rules.forEach((item) => {
      if (item && item.oneOf) {
        //record the css-loader index
        let cssIndex = 0;
        item.oneOf.forEach((rule, index) => {
        ...
        //insert new css-loader for modules
        if (cssIndex != 0) {
          let lessLoaderUnModules = {
            test: /\.less$/,
          ...
          item.oneOf.splice(cssIndex, 0, lessLoaderUnModules);
          item.oneOf.splice(cssIndex + 1, 0, lessLoaderModules);
        }
        ...
        item.oneOf.forEach((rule, index) => {
          ...
          if (rule.test && rule.test.source) {
            ...
            if (rule.test.source.includes('\.less$')) {
              let lessLoader = {
